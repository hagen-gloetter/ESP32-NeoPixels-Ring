...
class WifiConnect:
    ...
    def connect(self):
        ...
        try:
            ...
            print(f"!!!!!!!!!!!!!!!! ERROR !!!!!!!!!!!!!!!! Cannot load {fn_secrets}: {e}")
            ...
            nets = self.wifi.scan()
            # Decode SSID bytes from scan tuples rather than stringifying the
            # whole list — prevents false positives from partial string matches.
            scan_ssids = []
            for net in nets:
                try:
                    scan_ssids.append(net[0].decode("utf-8"))
                except Exception:
                    pass
