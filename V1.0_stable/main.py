...
def on_message(topic, msg):
    ...
    if "totalsolarw" in topic:
        try:
            ...
            # V2.0 abs watt seems to be better than %
            num_leds = int( (float(round(int(msg), -2))+0.01) / 100 ) 
            if num_leds > 12:
                num_leds=12 # no overflow
            if num_leds<1:
                num_leds=0
            print (f"totalsolarw num_leds={num_leds}")
            led_ring2.set_single_color(num_leds, "g")
        except Exception as e:
            print("MQTT Error for totalsolarw. ")
            print("Received message `{0}` under topic {1}".format(msg, topic))
            led_ring2.set_single_color(1,"b")
    elif "acoutw" in topic:
        try:
            acoutw = (int(msg)) 
            # gives values from 0 (not possible) to 4200w (2500w) is normal max therefore 100%
            p = (int(msg)+1) * 100 / 2500
            if p < 0:
                p = 0
            if p > 100:
            ...
            # V2.0 abs watt seems to be better than %
            num_leds = int( (float(round(int(msg), -2))+0.01) / 100 ) 
            print (f"acoutw num_leds={num_leds}")
            if num_leds > 12:
                num_leds=12 # no overflow
            if num_leds<1:
                num_leds=0
            ...
            print("MQTT Error for acoutw. ")
            print("Received message `{0}` under topic {1}".format(msg, topic))
            led_ring2.set_single_color(2,"b")        
    else:
        SOC2 = SOC1
        SOC1 = float(msg)
        if SOC2 == 0:
        ...
        #        SOC2=int(msg)
        SOC = (SOC1+SOC2+0.01)/2  # no div/0
        #print(f"SOC1 {SOC1} SOC2 {SOC2} SOC={SOC}%")
        print (f"SOC num_leds={SOC}")
        led_ring1.display_percentage1(SOC)
    apache.set_variables(SOC1, acoutw, totalsolarw, mytime)
